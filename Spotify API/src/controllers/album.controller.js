...
export const createAlbum = async (req, res) => {
    ...
    try {
        ...
        session.endSession();
        res.status(201).json({ success: true, album: newAlbum[0] });
    }
    catch (err) {
...
export const getAlbums = async (req, res) => {
    ...
    try {
        ...
        const albums = await AlbumModel.find().limit(12).select('title cover artist').populate('artist', 'username');
...
export const getAlbum = async (req, res) => {
    const user = req.user;
    try {
        const album = await AlbumModel.findById(req.params.id).select('title cover artist').populate('artist', 'username');
        if (!album) {
...
export const deleteAlbum = async (req, res) => {
    ...
    if (album.artist.toString() !== user._id.toString()) {
    ...
    session.startTransaction();
    try {
        await AlbumModel.deleteOne({ _id: id }, { session });
